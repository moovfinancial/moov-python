...
class ListConnectedAccountsForAccountRequest(BaseModel):
    ...
    def serialize_type(self, value):
        if isinstance(value, str):
            try:
                return components.CreateAccountType(value)
            except ValueError:
                return value
        return value

    @field_serializer("capability")
    def serialize_capability(self, value):
        if isinstance(value, str):
            try:
    ...
    @field_serializer("capability_status")
    def serialize_capability_status(self, value):
        if isinstance(value, str):
            try:
...
class ListConnectedAccountsForAccountResponse(BaseModel):
    headers: Dict[str, List[str]]
