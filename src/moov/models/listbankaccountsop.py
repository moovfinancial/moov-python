"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .bankaccountresponse import BankAccountResponse, BankAccountResponseTypedDict
from moov.types import BaseModel
from moov.utils import FieldMetadata, PathParamMetadata
import pydantic
from typing import Dict, List, Union
from typing_extensions import Annotated, TypedDict


class ListBankAccountsRequestTypedDict(TypedDict):
    account_id: str
    r"""ID of the account."""


class ListBankAccountsRequest(BaseModel):
    account_id: Annotated[
        str,
        pydantic.Field(alias="accountID"),
        FieldMetadata(path=PathParamMetadata(style="simple", explode=False)),
    ]
    r"""ID of the account."""


ListBankAccountsResponseResultTypedDict = Union[List[BankAccountResponseTypedDict], str]


ListBankAccountsResponseResult = Union[List[BankAccountResponse], str]


class ListBankAccountsResponseTypedDict(TypedDict):
    headers: Dict[str, List[str]]
    result: ListBankAccountsResponseResultTypedDict


class ListBankAccountsResponse(BaseModel):
    headers: Dict[str, List[str]]

    result: ListBankAccountsResponseResult
