"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .schedule import Schedule, ScheduleTypedDict
from moov.types import BaseModel
from moov.utils import FieldMetadata, PathParamMetadata
import pydantic
from typing import Dict, List, Union
from typing_extensions import Annotated, TypedDict


class ListSchedulesRequestTypedDict(TypedDict):
    account_id: str
    r"""ID of the account."""


class ListSchedulesRequest(BaseModel):
    account_id: Annotated[
        str,
        pydantic.Field(alias="accountID"),
        FieldMetadata(path=PathParamMetadata(style="simple", explode=False)),
    ]
    r"""ID of the account."""


ListSchedulesResponseResultTypedDict = Union[List[ScheduleTypedDict], str]


ListSchedulesResponseResult = Union[List[Schedule], str]


class ListSchedulesResponseTypedDict(TypedDict):
    headers: Dict[str, List[str]]
    result: ListSchedulesResponseResultTypedDict


class ListSchedulesResponse(BaseModel):
    headers: Dict[str, List[str]]

    result: ListSchedulesResponseResult
